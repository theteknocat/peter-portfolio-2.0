...
- **Page/item body content is baked into the prerendered HTML**, including detail routes.
  `usePageData`/`useContent` (`src/composables/`) are `async` and are `await`-ed directly in
  `<script setup>` (async setup), which requires a `<Suspense>` ancestor and runs during the
  Node-side prerender — see `ExperienceView.vue` for the pattern. There is no
  client-only-hydrated placeholder step; don't reintroduce one without checking this first.
- `frontend/vite.config.ts` drives prerendering beyond the vite-ssg default:
  - `includedRoutes()` — fetches `/api/page/portfolio` and `/api/page/articles` at build time
    and appends `/portfolio/{slug}` and `/articles/{slug}` for every item, so dynamic detail
    routes actually get prerendered (vite-ssg only prerenders static routes by default).
  - `injectStructuredData()` / `injectJsonLd()` — after prerender, fetches page data again and
    writes a `<script type="application/ld+json">` block into each generated HTML's `<head>`:
    schema.org `Person` on `index.html`, `CreativeWork` per portfolio item, `Article` per
    article item.
  - `onFinished()` also writes `sitemap.xml` from the actual prerendered `.html` files found in
    `outDir` (via `htmlRoutes()`), so it can't drift from what was really generated.
  - Both build-time fetchers call `http://localhost/api/page/{page}` directly — they only work
    where the API is reachable at build time (`ddev exec`/CI), not a generic relative `fetch`.
- **Browser-only libraries must not run during the prerender.** For libraries that touch
  `window`/`document` (e.g. `clippyjs`), use a **dynamic `import()` inside `onMounted`**, never
  a top-level import. Type-only imports are fine (erased at build).
...
- **Deploy:** `ddev deploy` runs `deploy-content` then `deploy-frontend`
  (`.ddev/commands/host/`). `deploy-frontend` builds with `VITE_OUT_DIR=dist-prod` (keeps prod
  build output separate from the local dev `dist/`) and rsyncs `dist-prod/` to the server's
  `dist/`; `deploy-content` rsyncs `content/` separately. The server runs nginx + PHP-FPM only
  (no Node). **Content-only edits now require a full rebuild + `deploy-frontend`**, not just
  `deploy-content` — content is baked into the prerendered HTML, so rsyncing `content/` alone
  updates the API's source of truth but not the static pages already built from it. See
  `docs/architecture.md` § Build & Deploy.
...
## Content Markdown Bodies

Portfolio/article items *and* page sections (`content/sections/{page}/{slug}.yaml`) that need a
markdown body follow the same convention: the `.yaml` holds only metadata (e.g. `template: text`),
and a sibling `{slug}.md` file holds the prose. `ContentService::getItem()` auto-attaches the
sibling `.md` as the body — `PageHandler` calls the same method for sections as it does for
portfolio/article items, so this applies uniformly across both. Don't inline `body: |-` in the
yaml for markdown content — that pattern only exists for `home-intro.yaml`'s non-markdown `intro`
template. See `content/sections/portfolio/intro.yaml` + `intro.md` for the reference example.

...
> **Full guide: [`docs/clippy-companion.md`](clippy-companion.md)** — frontend capabilities, SSG constraints, the agent API reference, and the Claude API back-end design all live there. The notes below are just the headline decisions.

**Library:** `clippyjs` (`github.com/pi0/clippyjs`) — pi0's modern ESM rewrite. Installed and mounted. TypeScript types included; the agent sprite sheets and classic sounds are **bundled in the package**, so there's nothing to self-host and no CDN dependency.

> The earlier "pithings / `speakStream()`" recommendation is superseded — that package isn't on npm, and pi0's `clippyjs` has no `speakStream()`. Streaming is unnecessary anyway: `speak()` is one-shot and already animates text word-by-word itself (see the back-end design in the dedicated doc).

**Resolved (was "needs investigation"):**

- **Vue 3:** wrapped in `ClippyCompanion.vue`; the library injects its own DOM into `<body>`.
- **SSG:** the library is browser-only, so it's loaded via dynamic `import()` inside `onMounted` to keep it out of the vite-ssg prerender. Dispose in `onUnmounted`.
- **Assets:** bundled in the package — no self-hosting or CDN.
- **Theming:** balloon DOM uses the library's own classes; restyle via global unlayered CSS.

### Vue Component Shape

See **[`docs/clippy-companion.md`](clippy-companion.md)** for the component shape, the route-reactive pattern, and the full back-end flow. In short: `ClippyCompanion.vue` watches the route and speaks a line (canned, or — later — generated by the Claude API from page context); `ClippyMessage.vue` holds the canned action buttons, which are defined per-route on the frontend, **not** returned by the API.
